...
- **Complexity Degradation**: Lookup complexity degrades from the desired logarithmic time ($O(\log N)$) to linear time ($O(N)$).
...
- **Logarithmic Complexity**: In a balanced tree, following a pointer cuts the remaining search space in half on average, guaranteeing $O(\log N)$ lookups.
...
While balanced binary trees provide $O(\log N)$ lookups on average, their **low fanout** (maximum children per node is two) makes them impractical for disk-based storage:
...
- **Tree Height Problem**: A low fanout of two means the tree height is $\log N$. Finding an element requires $O(\log N)$ disk seeks and page transfers.
...
> 5. **Step 5: Cascading Splits (If needed)**: If the parent node was already full, the parent node will also split, promoting a key to _its_ parent. This can cascade all the way to the root!
