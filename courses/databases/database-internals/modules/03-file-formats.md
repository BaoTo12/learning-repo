...
*   **How it Solves the Goals**:
    *   *Minimal Overhead*: Only requires a small pointer array at the start of the page.
    *   *Defragmentation*: The page can be rewritten and cells compacted internally without changing the external **Slot IDs**.
    *   *Indirection*: Pointers can be re-sorted or updated internally; the Slot ID remains unchanged.
...
1.  **Initial State**: Inserting "Tom", then "Leslie":
    *   *Physical cells (appended)*: `[Tom]` (rightmost), then `[Leslie]` (to its left).
    *   *Logical pointers (sorted)*: The pointer array points to `[Leslie]` first, then `[Tom]`.
2.  **Adding a New Key**: Inserting "Ron":
    *   *Physical cell*: Appended to the remaining free space.
    *   *Pointers re-sorted*: Pointers after the insertion point are shifted to make room for the new pointer in sorted order: `[Leslie]`, `[Ron]`, `[Tom]`.
