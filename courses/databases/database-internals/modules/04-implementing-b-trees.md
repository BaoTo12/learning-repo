...
>    * *Scenario A (Parent has room)*: The database writes the separator key to Page C. The split is complete!
>    * *Scenario B (Parent is full)*: Page C overflows and must split into C and G.
> 5. **Step 5: Cascade Upward**: Since Page C split, we must insert a separator key into *its* parent. The database pops the next page from the stack: `Internal Page B`. This process repeats.
