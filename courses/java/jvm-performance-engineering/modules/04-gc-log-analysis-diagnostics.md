...
To understand garbage collection behavior, we must learn to read raw GC logs. Let us first dissect a traditional, pre-JDK 9 GC log entry to learn the baseline parameters before exploring the Unified Logging specification.

### Dissecting a Classic GC Log Line
Below is a standard young generation GC log line generated by the Parallel Garbage Collector:

```
[GC (Allocation Failure) [PSYoungGen: 59392K->8192K(61440K)] 102400K->61440K(204800K), 0.0123450 secs] [Times: user=0.03 sys=0.01, real=0.01 secs]
```

Let us break down each component of this log signature:
1.  **`GC (Allocation Failure)`**: The GC event type and cause. An "Allocation Failure" means Eden space has run out of space for new objects, triggering a collection.
2.  **`[PSYoungGen: 59392K->8192K(61440K)]`**:
    *   `PSYoungGen`: The Young Generation collector name (Parallel Scavenge Young).
    *   `59392K->8192K`: Young Generation memory utilization *before* collection (59.3MB) and *after* collection (8.1MB).
    *   `(61440K)`: The total configured size of the Young Generation.
3.  **`102400K->61440K(204800K)`**:
    *   `102400K->61440K`: Total Heap occupancy *before* (102.4MB) and *after* (61.4MB) collection. The difference represents the objects reclaimed during this cycle (102.4 - 61.4 = 41MB of garbage cleared).
    *   `(204800K)`: The total configured Heap size.
4.  **`0.0123450 secs`**: The wall-clock duration of the Stop-The-World (STW) pause. During this time, all application threads were halted.
5.  **`[Times: user=0.03 sys=0.01, real=0.01 secs]`**:
    *   `user`: Total CPU time spent by GC threads in user space across all cores.
    *   `sys`: CPU time spent in operating system kernel calls.
    *   `real`: Actual elapsed physical time (matches the 0.0123450 secs pause time rounded). If `user` is significantly higher than `real` (e.g. 0.03 vs 0.01), it confirms the GC ran using multiple parallel worker threads.

### Transition to Unified Logging (`-Xlog`)
