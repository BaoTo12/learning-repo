...
import com.example.grpc.routeguide.RouteSummary;
import reactor.core.publisher.Flux;
...
public class ReactivePipelineRunner {
    ...
    public static Flux<Point> getDbPointsFlux() {
        return Flux.range(1, 100)
            .map(i -> Point.newBuilder().setLatitude(i).setLongitude(i * 2).build())
            .subscribeOn(Schedulers.boundedElastic()); // Run on IO-bound threads
    }

    public static void streamFluxToGrpc(Flux<Point> pointsFlux, StreamObserver<Point> requestObserver) {
        ClientCallStreamObserver<Point> clientCallObserver = (ClientCallStreamObserver<Point>) requestObserver;
        ...
        pointsFlux.subscribe(
            new reactor.core.CoreSubscriber<Point>() {
    ...
    public static void main(String[] args) throws InterruptedException {
        Flux<Point> dbPoints = getDbPointsFlux();
        CountDownLatch latch = new CountDownLatch(1);

        // Mock gRPC ClientCallStreamObserver for demonstration
        StreamObserver<Point> mockObserver = new ClientCallStreamObserver<>() {
            private int count = 0;
            @Override public void onNext(Point value) {
                count++;
                System.out.printf("Sent point #%d over gRPC wire: Latitude: %d\n", count, value.getLatitude());
            }
